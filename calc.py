...
import json # Un fichier JSON est un fichier qui stocke des structures de données simples et des objets,Il est principalement utilisé pour transmettre des données entre un serveur et une application Web, comme alternative à XML
#  ╔═══════════════════════════════════════╗
#  ║          Calculatrice Python          ║
#  ║               Crée par                ║
...
history_file_path = 'Historique.json'

root.geometry("550x430")
root.title("Calcultrice ")
root.resizable(0, 0)
...
def display_history():
    ...
    index = 0
    while index < len(history): # La boucle while parcours chaque élément de l'historique et les insère dans la liste widget à chaque itération.
        history_list.insert(index, history[index])
        index += 1


history_button = Button(root, text="Historique", command=display_history,background="#708090")
history_button.grid(row=6, column=3)

percentage_button = Button(root, text="%", command=Calculate)
...
clear_history_button = Button(root, text="Effacer l'historique", command=clear_history,background="#708090")
